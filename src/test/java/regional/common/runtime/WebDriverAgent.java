...
public class WebDriverAgent {
    ...
    public JsonObject waitFor(Supplier<JsonObject> action, Predicate<JsonObject> checker, String error_template) {
        ...
        while (System.currentTimeMillis() - start < Long.parseLong(TestProperty.getProperty("environment", "timeout")) * 1000) {
    ...
    public void openApp(String device, String region) {
        String body = "{\"desiredCapabilities\":{\"bundleId\":\"" + bundleId.get(region) + "\"}}";
    ...
    public void cleanKeychain(String device, String region) {
        try {
            Runtime.getRuntime().exec(commandPath + " " + deviceInfo.get(device) + " clear_keychain " + bundleId.get(region));
        } catch (Exception e) {
            e.printStackTrace();
        }
        sleepInterval("1");
    ...
    public void installApp(String device, String region) {
        try {
            Runtime.getRuntime().exec(commandPath + " " + deviceInfo.get(device) + " install " + appPath.get(region));
        } catch (Exception e) {
            e.printStackTrace();
        }
        sleepInterval("2");
    ...
    public void uninstallApp(String device, String region) {
        try {
            Runtime.getRuntime().exec(commandPath + " " + deviceInfo.get(device) + " uninstall " + bundleId.get(region));
        } catch (Exception e) {
            e.printStackTrace();
        }
        sleepInterval("3");
    ...
    public void cleanEnv(String device, String region) {
        cleanKeychain(device, region);
        uninstallApp(device, region);
        installApp(device, region);
    }
    ...
    public void sleepInterval(String sec) {
        try {
